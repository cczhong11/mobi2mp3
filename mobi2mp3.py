...
class Mobi(object):
    ...
    def to_txt(self):
        ...
        if os.path.exists(f"{self.outputpath}/{self.bookname}.txt"):
            return
        os.system(
            "/usr/local/bin/ebook-convert {0} {2}/{1}.txt".format(
                self.filename, self.bookname, self.outputpath
            )
        )
        ...
        with open(self.outputpath + "/" + self.bookname + ".txt") as f:
            ...
            for line in f.readlines():
                ...
                if count > 10:
                    ...
                    tts.save("result-{0}.mp3".format(str(total_count).zfill(4)))
                    filelist.append("result-{0}.mp3".format(str(total_count).zfill(4)))
                    count = 0
                    total_count += 1
                    string = ""
    ...
    def concat_mp3(self):
        os.system(
            "/usr/local/bin/ffmpeg -f concat -i result.txt -c copy {0}.mp3".format(
                self.bookname
            )
        )
        ...
        with open(self.outputpath + "/" + self.bookname + ".txt") as f:
            ...
            for line in f.readlines():
                ...
                if len(line) > 0:
                    string += line
                    count += len(line)
                if (count > 5000 and self.language == "zh_CN") or (
                    count > 8000 and self.language == "en_US"
                ):
                    ...
                    string = ""
            if not os.path.exists(f"result-{total_count}.aiff"):
                engine.save_to_file(string, "result-{0}.aiff".format(total_count))
            res.append("result-{0}.aiff".format(total_count))
...
def test_aiff():
    book = Mobi("test.mobi", "en_US", ".", "osx", 200)
    book.run()


def test_mp3():
    book = Mobi("test.mobi", "en_US", ".", "osx")
    book.run()
...
def main():
    ...
    parser.add_argument(
        "-f", "--format", help="format setting like gtts/osx", default="osx"
    )
